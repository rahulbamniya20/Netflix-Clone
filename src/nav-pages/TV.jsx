...
const TV = () => {
  ...
  const apiUrl = `https://api.themoviedb.org/3/search/tv?api_key=${APIKEY}&language=en-US&page=1&include_adult=false&query=${searchMovie}`;
  const movieUrl = `https://api.themoviedb.org/3/tv/popular?api_key=${APIKEY}&language=en-US`;
  ...
  return (
    <>
      ...
      <div className="movie__posters">
        {movies &&
          movies.map((movie) => (
            <Link to={`/tv/${movie.id}`} className="movie_link" key={movie.id}>
              <img
                // onClick={() => handleClick(movie)}
                loading="lazy"
                src={`${baseImgUrl}${movie.poster_path}`}
